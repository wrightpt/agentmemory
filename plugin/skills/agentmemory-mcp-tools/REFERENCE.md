...
agentmemory exposes 53 MCP tools. Seven non-LLM essentials are in the lean core set (`--tools core` or `AGENTMEMORY_TOOLS=core`); the rest load with `--tools all` (default). `AGENTMEMORY_TOOLS=workstation` adds lessons, actions, leases, and signals while omitting LLM-backed tools.
